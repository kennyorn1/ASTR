...
class Nosiy_NN(nn.Module):
    def __init__(
        self, node_list, n_variable, n_output, noise_type="norm", noise_scale=1
    ):
        ...
        for i in range(len(node_list)):
            if i == 0:
                self.layer_list.append(
                    Sequential(nn.LSTM(n_variable, node_list[0], batch_first=True))
                )
            else:
                self.layer_list.append(
                    Sequential(
                        nn.LSTM(node_list[i - 1], node_list[i], batch_first=True)
                    )
                )
    ...
    def forward(self, traindata):
        if self.noise_type == "norm":
            noise = torch.from_numpy(
                self.noise_scale
                * np.random.normal(loc=0, scale=1, size=traindata.shape)
            )
            ...
            noise = torch.from_numpy(
                self.noise_scale * (np.random.random(traindata.shape))
            )
            if torch.cuda.is_available():
                traindata = traindata.cuda()
                noise = noise.cuda()
            ...
            noise = torch.from_numpy(
                self.noise_scale
                * np.sqrt((-1 * np.log(1 - np.random.random(traindata.shape))))
            )
            if torch.cuda.is_available():
                traindata = traindata.cuda()
                noise = noise.cuda()
        ...
        for i in range(len(self.layer_list)):
            ...
            if i == len(self.layer_list) - 1:
                ...
                noisy_output = self.layer_list[i](
                    noisy_traindata[:, -1, :].squeeze().float()
                )
            else:
...
class lstm_network:
    def __init__(
        self, node_list, n_variable, n_output, noise_type="norm", reg=0, noise_scale=1
    ):
        self.reg = reg
        self.model = Nosiy_NN(
            node_list,
    ...
    def fit(self, traindata, trainlabel, batch_size=128, epochs=200, lr=1e-3):
        ...
        for epoch in range(epochs):
            ...
            print(
                ...
                "loss_train: {:.8f}".format(acc_loss.item()),
            )
...
class Nosiy_NN_regressor:
    def __init__(
        self, node_list, n_variable, n_output, noise_type="norm", reg=0.1, noise_scale=1
    ):
        self.reg = reg
        self.model = Nosiy_NN(
            node_list,
    ...
    def fit(self, traindata, trainlabel, batch_size=128, epochs=200, lr=1e-3):
        ...
        for epoch in range(epochs):
            ...
            print(
                "Epoch: {:04d}".format(epoch + 1),
                "loss_train: {:.8f}".format(acc_loss.item()),
                "loss_fit: {:.8f}".format(acc_loss1.item()),
                "loss_reg: {:.8f}".format(acc_loss2.item()),
            )
        print("Optimization Finished!")
    ...
    def fit(self, traindata, trainlabel, udata, batch_size=64, epochs=200, lr=1e-3):
        ...
        for epoch in range(epochs):
            ...
            for item, u_item in zip(train_dataloader, u_dataloader):
                ...
                train_loss1 = loss1(output, batch_label.float())
                train_loss2 = loss2(
                    torch.cat([output, uoutput], dim=0),
                    torch.cat([noisy_output, noisy_uoutput], dim=0),
                )
                train_loss = train_loss1 + self.reg * train_loss2
