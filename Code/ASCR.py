...
class ASCR:
    ...
    def fit(self, traindata, trainlabel, udata):
        ...
        for t in range(self.T):
            for j in [0, 1]:
                ...
                for i in range(udata_pool.shape[0]):
                    neighbor_data, neighbor_label = self.k_neighbors(
                        udata_pool[i], traindata, trainlabel, k_list[j], d_list[j]
                    )
                    ulabel_list.append(h_list[j].predict(np.mat(udata_pool[i])))
                    new_h = KNeighborsRegressor(n_neighbors=k_list[j], metric=d_list[j])
                    new_h.fit(
                        np.vstack([traindata_list[j], np.mat(udata_pool[i])]),
                        np.vstack(
                            [
                                trainlabel_list[j],
                                h_list[j].predict(np.mat(udata_pool[i])),
                            ]
                        ),
                    )

                    error_old = mean_squared_error(
                        neighbor_label, h_list[j].predict(neighbor_data)
                    )
                    error_new = mean_squared_error(
                        neighbor_label, new_h.predict(neighbor_data)
                    )
                ...
                if max(sigma_list) > 0:
                    ...
                    traindata_list[1 - j] = np.vstack(
                        [traindata_list[1 - j], np.mat(udata_pool[useful_index])]
                    )
                    trainlabel_list[1 - j] = np.vstack(
                        [trainlabel_list[1 - j], np.mat(ulabel_list[useful_index])]
                    )
            ...
            if udata_pool.shape[0] == self.s:
                ...
                udata, udata_pool, _, _ = train_test_split(
                    udata, udata, test_size=self.s
                )

        self.regressor1 = Semisupervised_Nosiy_NN_regressor(
            node_list=[1024, 256],
            n_variable=34,
            n_output=1,
            noise_type="norm",
        ...
        self.regressor2 = Semisupervised_Nosiy_NN_regressor(
            node_list=[1024, 256],
            n_variable=34,
            n_output=1,
            noise_type="ruili",
        ...
        self.regressor1.fit(
            traindata1, trainlabel_list[0], udata, batch_size=32, epochs=50, lr=1e-4
        )
